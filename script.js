...
function startGame() {
  ...
  // Запускаем эффект — карточка уже прозрачная когда экран открывается
...
function playEntryEffect(cardEl, callback) {
  ...
  let meta = ENTRY_EFFECT_META[effectId];

  // Задержка 2 сек чтобы эффект успел прогрузиться
  setTimeout(() => {
    // Восстанавливаем видимость — теперь запускаем анимацию
    cardEl.style.opacity = "1";
    cardEl.style.transition = "";
    cardEl.style.visibility = "";

    // Убираем возможные предыдущие классы/элементы
    Object.values(ENTRY_EFFECT_META).forEach(m => {
      cardEl.classList.remove(m.cssClass);
    });
    cardEl.classList.remove('entry-active');

    // Удаляем старые доп. элементы
    let oldWave = cardEl.querySelector('.entry-impact-line');
    let oldFlash = cardEl.querySelector('.entry-anim-rift-flash');
    let oldRing = cardEl.querySelector('.entry-radiance-ring');
    ...
    // Добавляем overflow:visible для эффектов выходящих за рамки карточки
    cardEl.classList.add('entry-active');
    // Добавляем класс анимации
    cardEl.classList.add(meta.cssClass);

    // Доп. элементы
    if (meta.hasWave) {
      let wave = document.createElement('div');
      wave.className = 'entry-impact-line';
    ...
    if (meta.hasFlash) {
      ...
      flash.className = 'entry-anim-rift-flash';
      cardEl.style.position = 'relative';
    ...
    if (meta.hasRing) {
      ...
      ring.className = 'entry-radiance-ring';
      cardEl.style.position = 'relative';
    ...
    let duration = meta.cssClass === 'entry-anim-rift' ? 950 :
                   meta.cssClass === 'entry-anim-radiance' ? 900 :
                   meta.cssClass === 'entry-anim-materialize' ? 850 : 650;

    setTimeout(() => {
      cardEl.classList.remove(meta.cssClass);
      cardEl.classList.remove('entry-active');
      if (callback) callback();
    }, duration);
...
const MYTHIC_GACHA_POOLS = {
  ...
  darkknight: {
    ...
    frame:       { id: 'valinor',       name: '✨ Золото Валинора',        chance: 0.004 },
...
const FRAME_META = {
  lightning:   { name: '⚡ Молния',           rarity: 'epic',      class: 'frame-lightning' },
  blood:       { name: '🩸 Кровь Дракона',    rarity: 'epic',      class: 'frame-blood' },
  void:        { name: '🌌 Бездна',           rarity: 'legendary', class: 'frame-void' },
  astral:      { name: '🌠 Астрал',           rarity: 'legendary', class: 'frame-astral' },
  valinor:     { name: '✨ Золото Валинора',  rarity: 'mythic',    class: 'frame-valinor' },
  chaos:       { name: '💥 Хаос',             rarity: 'mythic',    class: 'frame-chaos' }
};

// Мета-данные эффектов появления
const ENTRY_EFFECT_META = {
  slide:        { name: '📈 Слайд снизу',         rarity: 'epic',      cssClass: 'entry-anim-slide',       hasFlash: false, hasWave: false, hasRing: false },
  flash:        { name: '⚡ Вспышка',              rarity: 'epic',      cssClass: 'entry-anim-flash',       hasFlash: false, hasWave: false, hasRing: false },
  materialize:  { name: '🌫️ Материализация',       rarity: 'legendary', cssClass: 'entry-anim-materialize', hasFlash: false, hasWave: false, hasRing: false },
  impact:       { name: '💥 Удар с небес',         rarity: 'legendary', cssClass: 'entry-anim-impact',      hasFlash: false, hasWave: true,  hasRing: false },
...
function openEntryEffectModal() {
  ...
  let rarityColors = { epic: '#a855f7', legendary: '#f59e0b', mythic: '#ef4444' };
  let rarityLabels = { epic: 'Эпический', legendary: 'Легендарный', mythic: 'Мифический' };

  // Сортируем по редкости
  let order = ['epic','legendary','mythic'];
...
function openCardFrameModal() {
  ...
  let rarityColors = { epic: '#a855f7', legendary: '#f59e0b', mythic: '#ef4444' };
  let rarityLabels = { epic: 'Эпическая', legendary: 'Легендарная', mythic: 'Мифическая' };
  let order = ['epic','legendary','mythic'];
...
function decodeProfile(code) {
  try {
    ...
    if (!data || data.v !== 1) return { ok: false, reason: 'Устаревший формат кода.' };
    return { ok: true, data };
  } catch(e) {

...
function updateScreen() {
  ...
  if (playerSubEl) playerSubEl.innerHTML = playerTitle ? getTitleHtml(playerTitle.rarity, playerTitle.name) : `<span style="color:#64748b;">${player.className}</span>`;
  ...
  if (bot.isMob) {
    ...
    if (botSubEl) {
      if (bot.activeTitle) botSubEl.innerHTML = getTitleHtml(bot.activeTitle.rarity, bot.activeTitle.name);
      else botSubEl.innerHTML = `<span style="color:#64748b;">${bot.className}</span>`;
    }
...
function rollGacha(gachaId) {
  ...
  if (result.type === 'title') {
    ...
    if (!td.unlocked.includes(result.rarity)) td.unlocked.push(result.rarity);
  }
...
function renderGachaList() {
  ...
  el.innerHTML = visible.map(g => {
    ...
    return `
    <div class="gacha-card" style="border:1px solid ${g.borderColor}; border-radius:14px; overflow:hidden; margin-bottom:12px; background:rgba(15,23,42,0.9);">
      <!-- Заголовок -->
      <div style="background:linear-gradient(135deg, ${g.color}22, ${g.color}44); padding:14px 16px; border-bottom:1px solid ${g.borderColor}44;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
          <div>
            <div style="font-size:11px; color:${g.color}; font-weight:bold; letter-spacing:1px; text-transform:uppercase; margin-bottom:4px;">Рулетка</div>
            <div style="font-size:15px; font-weight:900; color:#f1f5f9;">${g.icon} ${g.name}</div>
      ...
      <div style="padding:12px 16px; display:grid; grid-template-columns:1fr 1fr; gap:6px; border-bottom:1px solid #1e293b;">
        ${['uncommon','rare','epic','legendary'].map(r => {
          ...
          return `<div style="background:rgba(30,41,59,0.6); border-radius:8px; padding:8px; border:1px solid ${unlocked ? colors[r]+'44' : '#1e293b'};">
            <div style="font-size:9px; color:${colors[r]}; font-weight:bold;">${t.label.toUpperCase()} · ${pct}</div>
            <div style="font-size:11px; color:${unlocked ? '#f1f5f9' : '#334155'}; margin-top:2px;">${unlocked ? t.name : '???'}</div>
      ...
      <div style="padding:12px 16px;">
        <button onclick="openGachaModal('${g.id}')"
          style="width:100%; background:linear-gradient(135deg, ${g.color}, ${g.borderColor}); border:none; border-radius:10px; padding:13px; color:white; font-size:15px; font-weight:900; cursor:pointer; letter-spacing:0.5px;">
          🎲 Крутить — 10 💠
        </button>
  ...
  if (result.type === 'title') {
    ...
    mainHtml = `
      <div style="text-align:center; padding:10px 0 20px;">
        ...
        <div style="font-size:20px; font-weight:900; margin-bottom:16px;">${titleHtml}</div>

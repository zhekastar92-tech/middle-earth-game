...
function playEntryEffect(cardEl, callback) {
  ...
  // Задержка 2 сек — убираем класс-скрытие и запускаем анимацию
  setTimeout(() => {
    // Убираем скрытие — анимация сама управляет opacity через @keyframes
    cardEl.classList.remove('entry-hidden');

    // Убираем возможные предыдущие классы

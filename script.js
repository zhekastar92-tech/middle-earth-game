...
function openChest10Modal(items, chestType) {
  ...
  let itemsHtml = items.map(item => {
    ...
    return `<div style="display:flex; align-items:center; gap:8px; padding:6px 8px; border-radius:8px;
      background:rgba(0,0,0,0.3); border:1px solid ${c}44; margin-bottom:4px;">
      <div style="width:28px; height:28px; border-radius:50%; border:1px solid ${c};
        background:radial-gradient(circle, ${c}33, transparent); display:flex; align-items:center;
        justify-content:center; font-size:10px; font-weight:900; color:${c}; flex-shrink:0;">
        ${rarityLabel[item.rarity]}
      </div>
      <div style="flex:1; text-align:left;">
        <div class="text-${item.rarity}" style="font-size:12px; font-weight:bold;">
  ...
  document.getElementById('modal-desc').innerHTML = `
    <div style="text-align:center; margin-bottom:10px; font-size:12px; line-height:1.7;">${statsHtml}</div>
    <div style="max-height:320px; overflow-y:auto; padding-right:2px;">${itemsHtml}</div>
  `;
  document.getElementById('modal-actions').innerHTML = `
    <button class="action-btn" style="background:#22c55e; width:100%;" onclick="closeModal()">✓ Забрать всё в сумку</button>
  `;

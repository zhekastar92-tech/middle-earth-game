...
// Секрет для подписи профиля (отдельный от промокодов)
...
function updateBackupStatusLine() {
  ...
  try {
    ...
    if (!raw) {
      el.innerHTML = `<div style="font-size:11px; color:#64748b;">Ещё нет сохранённого кода. Совершите любое действие в игре — код создастся автоматически.</div>`;
      return;
    }
    let parsed = JSON.parse(raw);
    let ts = parsed.ts || null;
    if (!ts) { el.innerHTML = ''; return; }

    let now = Date.now();
    let diff = Math.floor((now - ts) / 1000); // секунды
    let agoStr = '';
    if (diff < 60)        agoStr = 'только что';
    else if (diff < 3600) agoStr = `${Math.floor(diff/60)} мин. назад`;
    else if (diff < 86400)agoStr = `${Math.floor(diff/3600)} ч. назад`;
    else                  agoStr = `${Math.floor(diff/86400)} дн. назад`;
    ...
    el.innerHTML = `
      <div style="display:flex; align-items:center; gap:6px; background:rgba(22,101,52,0.15); border:1px solid #166534; border-radius:8px; padding:7px 10px;">
        <span style="color:#4ade80; font-size:13px;">✅</span>
        <span style="font-size:11px; color:#4ade80;">Код актуален · обновлён <b>${agoStr}</b> <span style="color:#166534;">(${dateStr})</span></span>
      </div>`;
  } catch(e) {
    el.innerHTML = '';
  }
...
// ============================================================
// БИТОВАЯ УПАКОВКА ПРЕДМЕТОВ (v3)
// Каждый предмет → 7 символов base64url (41 бит)
// Схема:
//   id[12] rarity[3] slot[2] hp[4]
//   perk_type[3] perk_val[3] perk_charges[2]
//   uniq_type[3] uniq_val[3]
//   leg[1] leg_val[3]
//   classId[3]
// ============================================================

const _B64A = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
const _RE   = { common:0, uncommon:1, rare:2, epic:3, legendary:4 };
const _RD   = ['common','uncommon','rare','epic','legendary'];
const _SE   = { head:0, body:1, arms:2, legs:3 };
const _SD   = ['head','body','arms','legs'];
const _PE   = { none:0, heal_once:1, block_pierce:2, first_strike:3 };
const _PD   = ['none','heal_once','block_pierce','first_strike'];
const _UE   = { none:0, healBonus:1, blockBonus:2, ignoreBlock:3, dodge:4 };
const _UD   = ['none','healBonus','blockBonus','ignoreBlock','dodge'];
const _CE   = { '':0, warrior:1, assassin:2, guardian:3, priest:4, darkknight:5 };
...
function packItem(item) {
  ...
  let pt = item.perk ? (_PE[item.perk.type] ?? 0) : 0;
  let pv = item.perk ? Math.min(item.perk.val || 0, 7) : 0;
  let pc = (item.perk?.charges) ? Math.min(item.perk.charges, 3) : 0;
  push(pt, 3); push(pv, 3); push(pc, 2);
  let ut = item.unique ? (_UE[item.unique.type] ?? 0) : 0;
  let uv = item.unique ? (item.unique.type === 'dodge' ? 15 : Math.min(item.unique.val || 0, 7)) : 0;
  push(ut, 3); push(uv, 3);
...
function unpackItem(str) {
  ...
  let id      = pop(12);
  let rarity  = _RD[pop(3)] || 'common';
  let slot    = _SD[pop(2)] || 'head';
  let hp      = pop(4);
  ...
  if (lt) {
    item.legendary = { type:'blockStreakBonus', val:lv, desc:`[ЛЕГ] +${lv} к макс. блокам подряд.` };
  }
  item.classId = item.classId || null;
  item.name = generateItemName(rarity, slot, !!item.perk, !!item.unique, false, item.classId ? undefined : undefined);
  return item;
}

// Экипировка: 5 классов × 4 слота → строка "7chars,7chars,7chars,7chars" (пустой слот = ".")
...
function unpackEquip(packed) {
  ...
  Object.keys(packed).forEach(cls => {
    // Поддержка старого формата v2 (объект {h,b,a,g}) и нового v3 (строка)
    let val = packed[cls];
    if (typeof val === 'string') {
      let parts = val.split(',');
      r[cls] = {
        head: parts[0] && parts[0] !== '.' ? unpackItem(parts[0]) : null,
        body: parts[1] && parts[1] !== '.' ? unpackItem(parts[1]) : null,
        arms: parts[2] && parts[2] !== '.' ? unpackItem(parts[2]) : null,
        legs: parts[3] && parts[3] !== '.' ? unpackItem(parts[3]) : null
      };
    } else {
      r[cls] = {
        head: val.h ? unpackItem(val.h) : null,
        body: val.b ? unpackItem(val.b) : null,
        arms: val.a ? unpackItem(val.a) : null,
        legs: val.g ? unpackItem(val.g) : null
      };
    }
  });
  Object.keys(CLASSES).forEach(c => {
    if (!r[c]) r[c] = { head:null, body:null, arms:null, legs:null };
  });
  return r;
}

// Сжатие ключей данжа: {dusty_key:2, wood_key:1} → "dk:2,wk:1"
const _KE = { dusty_key:'dk', wood_key:'wk', ancient_key:'ak' };
const _KD = { dk:'dusty_key', wk:'wood_key', ak:'ancient_key' };
function packKeys(keys) {
  return Object.entries(keys).filter(([,v])=>v>0)
    .map(([k,v]) => (_KE[k]||k) + ':' + v).join(',');
}
function unpackKeys(str) {
  if (!str) return {};
  let r = {};
  str.split(',').forEach(p => { let [k,v] = p.split(':'); if(k) r[_KD[k]||k] = parseInt(v)||0; });
  return r;
}

// Сжатие зелий подсумка: [{type:'large',heal:20},...] → "L,M,S..."
const _POT = { small:'S', medium:'M', large:'L' };
const _POTD = { S:'small', M:'medium', L:'large' };
const _POTH = { small:8, medium:13, large:20 };
const _POTN = { small:'🧪 Малое зелье', medium:'🧪 Среднее зелье', large:'🧪 Большое зелье' };
function packPouch(pouch) {
  let items = (pouch.items||[]).map(p => _POT[p.type]||p.type).join('');
  return pouch.slots + '|' + items;
}
function unpackPouch(str) {
  if (!str) return { slots:0, items:[] };
  let [s, items] = str.split('|');
  return {
    slots: parseInt(s)||0,
    items: (items||'').split('').filter(Boolean).map(c => {
      let type = _POTD[c]||c;
      return { type, name: _POTN[type]||type, heal: _POTH[type]||0 };
    })
  };
}

// Сжатие прогресса данжей: {mansion:4,river:2} → "m:4,r:2"
const _DE = { mansion:'m', river:'r', temple:'t' };
const _DD = { m:'mansion', r:'river', t:'temple' };
function packDp(dp) {
  return Object.entries(dp).filter(([,v])=>v>0)
    .map(([k,v]) => (_DE[k]||k) + ':' + v).join(',');
}
function unpackDp(str) {
  if (!str) return {};
  let r = {};
  str.split(',').forEach(p => { let [k,v] = p.split(':'); if(k) r[_DD[k]||k] = parseInt(v)||0; });
  return r;
}

// Сжатие титулов: {guardian:{unlocked:['uncommon','rare'],active:'rare'}} → "g:ur:r"
const _TC = { uncommon:'u', rare:'r', epic:'e', legendary:'l', mythic:'m' };
const _TD = { u:'uncommon', r:'rare', e:'epic', l:'legendary', m:'mythic' };
const _CLS = { warrior:'w', assassin:'a', guardian:'g', priest:'p', darkknight:'d' };
const _CLSD = { w:'warrior', a:'assassin', g:'guardian', p:'priest', d:'darkknight' };
function packTitles(ttl) {
  return Object.entries(ttl).filter(([,v])=>v&&v.unlocked&&v.unlocked.length)
    .map(([cls,v]) => (_CLS[cls]||cls) + ':' + v.unlocked.map(r=>_TC[r]||r).join('') + ':' + (_TC[v.active]||v.active||'')).join(';');
}
function unpackTitles(str) {
  if (!str) return {};
  let r = {};
  str.split(';').forEach(p => {
    let [cls, ul, act] = p.split(':');
    let fullCls = _CLSD[cls]||cls;
    r[fullCls] = { unlocked:(ul||'').split('').map(c=>_TD[c]||c), active:_TD[act]||act||null };
  });
  return r;
}

// Собирает снапшот профиля для сохранения (v3 — максимальное сжатие)
function buildProfileSnapshot() {
  // Все численные поля кодируем компактно
  let snapshot = {
    v:   3,
    lp:  gameData.lp,
    ...
    gs:  _packSpins(gameData.gachaSpinCount),
    mt:  packTitles(gameData.mythicTitles),
    mg:  _packSpins(gameData.mythicGachaSpinCount),
...
function _packSpins(obj) {
  let s = Object.entries(obj).filter(([,v])=>v>0).map(([k,v])=>(_CLS[k]||k)+':'+v).join(',');
  return s || undefined;
}
function _unpackSpins(str) {
  if (!str) return {};
  let r = {};
  str.split(',').forEach(p => { let [k,v]=p.split(':'); if(k) r[_CLSD[k]||k]=parseInt(v)||0; });
  return r;
}

// Генерирует контрольную подпись профиля
function profileChecksum(payloadStr) {
  let str = payloadStr + PROFILE_SECRET;
  ...
  let combined = (Math.abs(h1) + Math.abs(h2)).toString(36).toUpperCase().padStart(8, '0');
  return combined.slice(0, 8);
}

// Кодирует профиль в строку ME-XXXX-XXXX-...
function encodeProfile() {
  try {
    let snapshot = buildProfileSnapshot();
    let json = JSON.stringify(snapshot);
    // Base64 через стандартный btoa (только latin1 — кодируем вручную через URI)
    let encoded = btoa(unescape(encodeURIComponent(json)));
    // Убираем символы не для URL/копирования
    let safe = encoded.replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
    // Добавляем подпись
    let checksum = profileChecksum(safe);
    let full = safe + checksum;
    // Разбиваем на блоки по 6 символов для читаемости: ME-XXXXXX-XXXXXX-...
    let blocks = [];
...
// Применяет декодированный профиль к gameData
function applyProfileSnapshot(data) {
  let v = data.v || 1;

  // --- Валюта и базовые поля ---
  gameData.lp           = data.lp  || 0;
  gameData.imperials    = (v >= 3 ? data.i   : data.imp)  || 0;
  gameData.lunarStones  = data.ls  || 0;
  gameData.maxInventory = (v >= 3 ? data.mi  : data.maxI) || 6;

  // --- Текущий класс ---
  if (v >= 3) {
    gameData.currentClass = _CLSD[data.c] || data.c || 'warrior';
  } else {
    gameData.currentClass = data.cls || 'warrior';
  }
  ...
  if (v >= 3) {
    ...
    gameData.inventory = (data.inv || []).map(unpackItem).filter(Boolean);
    gameData.equip     = unpackEquip(data.eq || {});
  } else {
    gameData.inventory = data.inv || [];
    gameData.equip     = data.eq  || {};
    if (gameData.equip && gameData.equip.warrior === undefined) {
      let flat = gameData.equip;
      gameData.equip = { warrior: flat };
    }
  }

  // --- Ключи и прогресс данжей ---
  if (v >= 3) {
    ...
    gameData.dungeonProgress = data.dp    || {};
    gameData.pouch           = data.pouch || { slots:0, items:[] };
  }

  // --- Гача и титулы ---
  gameData.hugeChestPity = data.pity || data.pt || 0;
  if (v >= 3) {
    gameData.titles               = unpackTitles(data.t  || '');
    gameData.gachaSpinCount       = _unpackSpins(data.gs || '');
    gameData.mythicTitles         = unpackTitles(data.mt || '');
    gameData.mythicGachaSpinCount = _unpackSpins(data.mg || '');
  } else {
    gameData.titles               = data.ttl  || {};
    gameData.gachaSpinCount       = data.gsc  || {};
    gameData.mythicTitles         = data.mtt  || {};
    gameData.mythicGachaSpinCount = data.mgsc || {};
  }
  ...
  if (v >= 3) {
    ...
    gameData.unlockedFrames         = data.uf  || [];
    gameData.unlockedEffects        = data.ue  || [];
    gameData.unlockedVictoryEffects = data.uve || [];
  }

...
// БАГ-ФИКС: "Подчинись мне" проверяется ПОСЛЕ боя (нужен реальный урон этого хода)
...
function playTurn(playerChoice) {
  ...
  // Проверяем умения моба ДО хода (только триггеры не зависящие от урона игрока)
  if (bot.isMob) {
  ...
  // БАГ-ФИКС триггера "Подчинись мне": проверяем ПОСЛЕ боя, когда lastPlayerDmgThisTurn уже заполнен
  if (bot.isMob) {
    logMsg += checkMobSubmitTrigger(bot, lastPlayerDmgThisTurn);
  }

  // Тикаем fury и immortal для бота (не игрока — игрок тикается после updateScreen)
  if (!bot.isMob) {
  ...
  // БАГ-ФИКС Болезни: canHeal обновляем ЗДЕСЬ (до эффектов), а не в середине хода
  // Это означает что болезнь применённая В ЭТОТ ХОД заблокирует лечение уже в эту же фазу эффектов
  if (bot.isMob) {
  ...
  // immortalTurnActive сбрасываем ПОСЛЕ всех эффектов (фикс смерти рыцаря от DoT)
  ...
  // БАГ-ФИКС бессмертия: уменьшаем счётчик ПОСЛЕ updateScreen
  // иначе кнопка "Возмездие" исчезает на ход раньше чем должна
...
function resolveCombat(atkC, defC, aRoll, dBlock, aName, dName, ignBlock, isSkill = false) {
  ...
  // БАГ-ФИКС: проверяем уклонение ДО вывода строки про блок
  if (!defC.isMob) {
    ...
    if (Math.random() < defC.eqP.dodge) {
      return res + `<span class="text-info">👢 Сапоги: ${dName} уклоняется!</span><br>`;
    }
  }

  // Уклонения не было — показываем блок

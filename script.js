...
// Собирает объект с данными профиля для сохранения
// Компактное кодирование предмета в строку вида "id|rarity|slot|hp[|perkType:val:charges][|uType:val][|lType:val][|classId]"
// Экономит ~70% размера по сравнению с JSON-объектом
function packItem(item) {
  if (!item) return null;
  let rarityMap = { common:'c', uncommon:'u', rare:'r', epic:'e', legendary:'l' };
  let slotMap   = { head:'h', body:'b', arms:'a', legs:'g' };
  let parts = [item.id, rarityMap[item.rarity]||item.rarity, slotMap[item.slot]||item.slot, item.hp];
  // Перк
  if (item.perk) {
    let pMap = { heal_once:'ho', block_pierce:'bp', first_strike:'fs' };
    let pc = pMap[item.perk.type] || item.perk.type;
    parts.push('p:' + pc + ':' + item.perk.val + (item.perk.charges != null ? ':' + item.perk.charges : ''));
  } else { parts.push(''); }
  // Уник
  if (item.unique) {
    let uMap = { healBonus:'hb', blockBonus:'bb', ignoreBlock:'ib', dodge:'dg' };
    let uc = uMap[item.unique.type] || item.unique.type;
    parts.push('u:' + uc + ':' + item.unique.val);
  } else { parts.push(''); }
  // Легендарный
  if (item.legendary) {
    parts.push('l:' + item.legendary.type + ':' + item.legendary.val);
  } else { parts.push(''); }
  // classId и name (name только если не null classId или нестандартное)
  parts.push(item.classId || '');
...
function unpackItem(str) {
  if (!str) return null;
  let p = str.split('|');
  let rarityMap = { c:'common', u:'uncommon', r:'rare', e:'epic', l:'legendary' };
  let slotMap   = { h:'head', b:'body', a:'arms', g:'legs' };
  let item = {
    id:     parseInt(p[0]),
    rarity: rarityMap[p[1]] || p[1],
    slot:   slotMap[p[2]]   || p[2],
    hp:     parseInt(p[3]),
    perk: null, unique: null, legendary: null, classId: null, name: ''
  };
  if (p[4]) {
    let pp = p[4].split(':');
    let pMap = { ho:'heal_once', bp:'block_pierce', fs:'first_strike' };
    let type = pMap[pp[1]] || pp[1];
    item.perk = { type, val: parseInt(pp[2]) };
    if (type === 'first_strike') item.perk.charges = parseInt(pp[3]) || 1;
    // Восстанавливаем desc
    if (type === 'heal_once')     item.perk.desc = `Лечит ${item.perk.val} ХП при падении здоровья.`;
    if (type === 'block_pierce')  item.perk.desc = `Блокирует ${item.perk.val} пробитого урона (1 раз).`;
    if (type === 'first_strike')  item.perk.desc = `Урон +${item.perk.val} на первые ${item.perk.charges} атак.`;
  }
  if (p[5]) {
    let up = p[5].split(':');
    let uMap = { hb:'healBonus', bb:'blockBonus', ib:'ignoreBlock', dg:'dodge' };
    let type = uMap[up[1]] || up[1];
    let val = parseFloat(up[2]);
  ...
  if (p[6]) {
    let lp = p[6].split(':');
    let val = parseInt(lp[2]);
    item.legendary = { type: lp[1], val };
    if (lp[1] === 'blockStreakBonus') item.legendary.desc = `[ЛЕГ] +${val} к макс. блокам подряд.`;
  }
  item.classId = p[7] || null;
  item.name    = p[8] || generateItemName(item.rarity, item.slot, !!item.perk, !!item.unique, false);
  return item;
}

// Компактная упаковка экипировки всех классов
...
function unpackEquip(packed) {
  ...
  Object.keys(packed).forEach(cls => {
    let eq = packed[cls];
    result[cls] = {
      head: unpackItem(eq.h),
      body: unpackItem(eq.b),
      arms: unpackItem(eq.a),
      legs: unpackItem(eq.g)
    };
  });
  // Миграция для классов без записи
  Object.keys(CLASSES).forEach(c => {
    if (!result[c]) result[c] = { head:null, body:null, arms:null, legs:null };
  });
  return result;
}
...
function buildProfileSnapshot() {
  return {
    ...
    inv:  gameData.inventory.map(packItem),
...
function applyProfileSnapshot(data) {
  ...
  gameData.imperials        = data.imp  || 0;
  gameData.lunarStones      = data.ls   || 0;
  gameData.currentClass     = data.cls  || 'warrior';
  gameData.maxInventory     = data.maxI || 6;

  // Поддержка v1 (старый формат) и v2 (сжатый)
  if (data.v === 2) {
    gameData.inventory = (data.inv || []).map(unpackItem).filter(Boolean);
    gameData.equip     = unpackEquip(data.eq || {});
  } else {
    gameData.inventory = data.inv || [];
    gameData.equip     = data.eq  || { warrior: { head:null, body:null, arms:null, legs:null } };
    // Миграция v1: equip может быть плоским объектом (до мультиклассовости)
    if (gameData.equip && gameData.equip.warrior === undefined && !gameData.equip.warrior) {
  ...
  gameData.hugeChestPity    = data.pity || 0;

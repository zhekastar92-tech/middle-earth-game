...
function startGame() {
  ...
  if (gameData.cardFrame && FRAME_META[gameData.cardFrame]) {
    playerCardClass += FRAME_META[gameData.cardFrame].class;
  } else {
...
function playEntryEffect(cardEl, callback) {
  ...
  setTimeout(() => {
    ...
    // Убираем анимационный класс после завершения и вызываем callback
    let duration = meta.cssClass === 'entry-anim-rift' ? 950 : meta.cssClass === 'entry-anim-radiance' ? 900 : meta.cssClass === 'entry-anim-materialize' ? 850 : 650;
    setTimeout(() => {
